/**
 * @fileoverview Clean E2E Test Suite
 *
...
describe("Clean E2E Test Suite", () => {
  ...
  describe("API Endpoint Tests", () => {
    // Debug logging
    console.log(`\n🔍 Debug Info:`);
    console.log(`   • apiNames: ${JSON.stringify(apiNames)}`);
    console.log(
      `   • discoveredEndpoints count: ${(discoveredEndpoints || []).length}`
    );
    console.log(
      `   • generatedConfigs keys: ${Object.keys(generatedConfigs || {}).join(", ")}`
    );
    ...
    console.log(
      `   • endpointsByApi keys: ${Object.keys(endpointsByApi).join(", ")}`
    );
