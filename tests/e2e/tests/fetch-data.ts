...
/**
 * Result of a fetch data test
 */
export interface FetchDataTestResult {
  success: boolean;
  message: string;
  data?: unknown;
  error?: string;
...
/**
 * Tests that fetch-dottie can successfully fetch data using default parameters
 */
async function testEndpointFetchData(
  endpoint: Endpoint<unknown, unknown>
): Promise<FetchDataTestResult> {
  const { api, functionName } = endpoint;

  try {
    const qualifiedFunctionName = `${api}:${functionName}`;
    testLogger.info(`Testing ${qualifiedFunctionName} data fetching`);

    // Call fetch-dottie with --no-validation flag to test basic data fetching
    ...
    // Parse the JSON output using JSON5 which is more tolerant of formatting issues
    let data: unknown;
    try {
    ...
    return {
      success: true,
      message: `Data fetching successful and returns useful data`,
      data,
    };
  } catch (error) {
