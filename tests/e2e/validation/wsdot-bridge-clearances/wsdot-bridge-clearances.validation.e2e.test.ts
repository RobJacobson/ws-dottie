...
describe("WSDOT Bridge Clearances API - Zod Validation", () => {
  it("should validate bridge clearances data structure using Zod", async () => {
    ...
    const validatedData = validators.bridgeClearancesArray.validateSafe(bridgeClearances);
    
    if (!validatedData.success) {
      console.error("Validation failed:", validatedData.error.errors);
      throw new Error(`Bridge clearances validation failed: ${JSON.stringify(validatedData.error.errors, null, 2)}`);
    }
    ...
    console.log(`✅ Successfully validated ${validatedData.data.length} bridge clearances`);
  });
  ...
  it("should validate individual bridge clearance data", async () => {
    ...
    if (bridgeClearances.length > 0) {
      ...
      const validatedBridge = validators.bridgeDataGis.validateSafe(firstBridge);
      
      if (!validatedBridge.success) {
        console.error("Individual validation failed:", validatedBridge.error.errors);
        throw new Error(`Individual bridge clearance validation failed: ${JSON.stringify(validatedBridge.error.errors, null, 2)}`);
      }
      ...
      // Test clearance data
      expect(validatedBridge.data.VerticalClearanceMaximumInches).toBeGreaterThan(0);
      expect(validatedBridge.data.VerticalClearanceMinimumInches).toBeGreaterThan(0);
      expect(typeof validatedBridge.data.VerticalClearanceMaximumFeetInch).toBe("string");
      expect(typeof validatedBridge.data.VerticalClearanceMinimumFeetInch).toBe("string");
    }
  });
  ...
  it("should handle nullable fields correctly", async () => {
    ...
    for (const bridge of bridgeClearances) {
      ...
      if (!validatedBridge.success) {
        console.error("Nullable validation failed:", validatedBridge.error.errors);
        throw new Error(`Nullable field validation failed: ${JSON.stringify(validatedBridge.error.errors, null, 2)}`);
      }
      ...
      if (validatedBridge.data.SRMPAheadBackIndicator !== null) {
        expect(typeof validatedBridge.data.SRMPAheadBackIndicator).toBe("string");
      }
    }
  });
  ...
  it("should validate data transformations automatically", async () => {
    ...
    if (bridgeClearances.length > 0) {
      ...
      if (validatedBridge.success) {
        ...
        const now = new Date();
        const apiLastUpdate = validatedBridge.data.APILastUpdate instanceof Date ? validatedBridge.data.APILastUpdate : new Date(validatedBridge.data.APILastUpdate);
        const routeDate = validatedBridge.data.RouteDate instanceof Date ? validatedBridge.data.RouteDate : new Date(validatedBridge.data.RouteDate);
